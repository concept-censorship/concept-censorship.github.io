...
from ldm.data.personalized import PersonalizedBase_eval
...
if __name__ == "__main__":
    ...
    data_loader = PersonalizedBase_eval(opt.data_dir, size=256, flip_p=0.0)
    ...
    if os.path.exists('./{opt.output_dir}.txt'):
        f = open(f'./{opt.output_dir}.txt', 'w')
    else:
        f = open(f'./{opt.output_dir}.txt', 'a')

    f.write(f'prompt: {opt.prompt}\n')    
    f.write(f'img score: {sim_img}\n')
    f.write(f'img score: {sim_text}\n\n')
    f.close()
